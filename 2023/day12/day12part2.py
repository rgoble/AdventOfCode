...
@cache
def getCombos(springs, data, currLen):
    ...
    if springs[0] == '.':
        if currLen == 0:
            ...
            return getCombos(springs[1:], data, 0)
            ...
            return getCombos(springs[1:], data[1:], 0)
        else:
        ...
        if currLen <= data[0]:
            ...
            return getCombos(springs[1:], data, currLen)
        else:
        ...
        if currLen == 0:
            ...
            sum = getCombos(springs[1:], data, 0)
        elif len(data) > 0 and currLen == data[0]:
            ...
            sum = getCombos(springs[1:], data[1:], 0)
        elif len(data) > 0 and currLen > data[0]:
            # Its invalid
            # print("Invalid #1 => '%s'" % s)
            return 0
        ...
        return sum + getCombos(springs[1:], data, currLen+1)
...
with open("input.txt", "rt") as inp:
    for line in inp:
        ...
        data = tuple(int(x) for x in data.split(',')) * 5
        # print("\tx5 springs = %s  ==> Data: %s" % (springs, data))

        # if line != "?###???????? 3,2,1":
        #     continue

        combos = getCombos(springs, data, 0)
        ans += combos
